...
export const extractUserAttributes = (tokenConfig: TokenResponseConfig): UserAttributes => {
    ...
    const nameClaims: { name: string | null; given_name: string | null; family_name: string | null } = Object.assign(
        { name: null, given_name: null, family_name: null },
        claims
    );

    if (nameClaims.name === null && (nameClaims.given_name !== null || nameClaims.family_name !== null)) {
        nameClaims.name = ((nameClaims.given_name || "") + " " + (nameClaims.family_name || "")).trimEnd();
    }
 ...
 */
export const LoginProvider = ({ children }: PropsWithChildren<LoginProviderProps>): React.ReactElement => {
    ...
    const wrappedSetAuthTokens = useCallback(
        async (authTokens: TokenResponseConfig | null) => {
            ...
            if (loggedIn) {
                ...
                await Promise.all([storeAuthTokens(authTokens), storeUserAttributes(extractedUserAttributes)]);
    ...
    useEffect(() => {
        ...
        async function fetchTokens() {
            const [tokens, attributes] = await Promise.all([retrieveAuthTokens(), retrieveUserAttributes()]);
            if (!active) {
                return;
            }
